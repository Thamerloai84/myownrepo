-- Rayfield Gen2 UI Migration + Revenge Fling (Updated)
...
local function hasTool(player, toolName)
    ...
    local success, result = pcall(function()
        ...
        if char and char:FindFirstChild(toolName, true) then return true end
        return false
    end)
...
local function findKiller()
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and hasTool(player, "Monster") then
            return player
        end
    end
    return nil
...
--======================== SKID FLING (R-77) ==============--
-- Ke intact from previous version in case you need to trigger it manually later
local SkidFling = function(TargetPlayer)
    ...
    if Character and Humanoid and RootPart and TCharacter then
        ...
        if THumanoid and THumanoid.Sit then return Message("Error Occurred", "Targeting is sitting", 5) end
...
local function clearESP(player)
    ...
    if char then
        for _, v in ipairs(char:GetChildren()) do
            if v.Name == "ESPHighlight" or v.Name == "ESPBillboard" then v:Destroy() end
        end
    end
end
...
local function createESP(player, role)
    ...
    if existing and existing.Role == role and existing.Highlight and existing.Highlight.Parent == char then return end
    clearESP(player)
    
    local hl = Instance.new("Highlight")
    hl.Name = "ESPHighlight"; hl.Adornee = char
    hl.FillColor = ROLE_COLORS[role]; hl.OutlineColor = Color3.new(1,1,1)
    hl.FillTransparency = 0.5; hl.OutlineTransparency = 0
    hl.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop; hl.Parent = char

    local bb = Instance.new("BillboardGui")
    bb.Name = "ESPBillboard"; bb.Adornee = char:FindFirstChild("Head") or root
    bb.Size = UDim2.new(0, 100, 0, 40); bb.StudsOffset = Vector3.new(0, 2.5, 0)
    ...
    lbl.TextColor3 = ROLE_COLORS[role]; lbl.TextStrokeTransparency = 0
...
RunService.Heartbeat:Connect(function()
    if not (Flags.ESP_Killer or Flags.ESP_Sheriff or Flags.ESP_Player) then
        for p in pairs(ESPObjects) do clearESP(p) end; return
    end
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then
            local isKiller  = hasTool(player, "Monster")
            local isSheriff = hasTool(player, "Gun")
            local shouldShow = false; local role = nil

            if isKiller and Flags.ESP_Killer then role = "Killer"; shouldShow = true
            elseif isSheriff and Flags.ESP_Sheriff then role = "Sheriff"; shouldShow = true
            elseif (not isKiller and not isSheriff) and Flags.ESP_Player then role = "Player"; shouldShow = true end

            if shouldShow then createESP(player, role) else clearESP(player) end
        end
    end
end)
            ...
            if holder and root then
                for _, obj in ipairs(holder:GetChildren()) do
                    if obj:IsA("MeshPart") and Flags.AutoCoins then
                        root.CFrame = obj.CFrame; task.wait(0.1)
                    end
                end
            end
        end
                    ...
                    if part then
                        ...
                        if root then
                            ...
                            while Flags.AutoGun and part.Parent and root.Parent do
                                part.CFrame = root.CFrame; task.wait(0.05)
                                if not part.Parent then break end 
                                part.CFrame = originalCFrame; task.wait(0.05)
                            end
                        end
                    end
                end
...
task.spawn(function()
    while true do
        if Flags.RevengeFling then
            ...
            local button = ghostText and ghostText:FindFirstChild("GoLobby")
            
            -- 1. Wait for GhostText and GoLobby to be visible
            if button and ghostText.Visible and button.Visible then
                
                -- 2. Trigger Activated listeners
                pcall(function()
                    if getconnections then
                        ...
                        -- Fallback if exploit doesn't support getconnections
                        button:FireServer() 
                    end
                end)
                
                -- 3. Wait to be respawned
                local char = LocalPlayer.CharacterAdded:Wait()
                local hrp = char:WaitForChild("HumanoidRootPart", 10)
                local hum = char:WaitForChild("Humanoid", 10)
                
                if hrp and hum and hum.Health > 0 then
                    task.wait(1) -- Small delay to ensure full spawn
                    ...
                    if killer and killer.Character and killer.Character:FindFirstChild("HumanoidRootPart") then
                        ...
                        -- 5. Fling the killer using BodyThrust
                        ...
                        -- Fling them until they are superflung (Head is gone) or 90 seconds pass
                        repeat
                            if not Target.Character or not Target.Character:FindFirstChild("HumanoidRootPart") then break end
                            if not hrp.Parent then break end -- Break if we die again
                            
                            hrp.CFrame = Target.Character.HumanoidRootPart.CFrame
                            Thrust.Location = Target.Character.HumanoidRootPart.Position
                            RunService.Heartbeat:Wait()
                        until not Target.Character:FindFirstChild("Head") or (tick() - startTime) > Duration
                ...
                task.wait(2) -- Cooldown before checking again
            end
        end
        task.wait(0.5)
    end
...
PlayerTab:CreateToggle({
    ...
    flag = "RevengeFling",
    description = "Auto respawns and flings the monster if you die.",
    value = false,
...
ESPTab:CreateToggle({
    ...
    callback = function(v)
        Flags.ESP_Killer = v
        if not v then for p, d in pairs(ESPObjects) do if d.Role == "Killer" then clearESP(p) end end end
    end
})
...
ESPTab:CreateToggle({
    ...
    callback = function(v)
        Flags.ESP_Sheriff = v
        if not v then for p, d in pairs(ESPObjects) do if d.Role == "Sheriff" then clearESP(p) end end end
    end
})
...
ESPTab:CreateToggle({
    ...
    callback = function(v)
        Flags.ESP_Player = v
        if not v then for p, d in pairs(ESPObjects) do if d.Role == "Player" then clearESP(p) end end end
    end
})
